...
### 1.1 Introduction 
For this analysis the brief set out was to analyze a real data and ask some interesting questions relating to the data. After searching through various websites this data set jumped out. It is a record of the number of people arrested at National Football League games starting in 2011 up until the 2015 season. Also included are 11 other factors such as the home score such as the visiting team, scores for both teams, the time the game was played etc.   
...
```
\newpage
```{r print data dictioanry, echo=FALSE}
...
### 2.2 Model Selection
Looking at the some of the graphs does offer some insight into what factors may be important or at the very least rule out the impact a variable such as `OT_flag` have. While these graphs do offer some insight and are interesting they do little to give quantifiable proof of the important factors. Model selection can be used to create prediction models as well as identify the important predictors. 
...
A grid of 100 values ranging from -2 to 10 is created this will test model with 100 different $\lambda$ and give the MSE for 100 different models enabling us to circle in on the best $\lambda$ for the model. If we set the model up from just the intercept, the MSE would be {r intercept}, where as the average across all $\lambda$ was {r mean.rr}. Fig. 7. graphs the log of $\lambda$ on the x-axis with the resulting MSE on the y-axis. 
...
After applying a number of methods to the data along with a exploration into the data which can be found here **Github**.
...
```{r Mean Gametime plot, echo=FALSE, fig.cap = "Fig. 3.3 Mean number of arrests of game time(Number of Kickoffs at this time)."}
